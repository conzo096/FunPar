		...
		if (myPlayerId == -1) {
			...
			while (enroled) {
				...
				playerIds.each { p ->
					...
					// Find name of player who's turn it is.
					if(p == turn)
						namedTurn = pData[0]
				}
				
				IPconfig.write("Playing Game Number - " + gameId + "| Current turn: "+namedTurn)
				...
				while ((chosenPairs[1] == null) && (enroled) && (notMatched))
				{
					...
					{
						// Handles messages from controller manager.
						case TURN:
							def o = fromController.read()
							if(o instanceof UpdateBoard)
							{
								def point = o.vPoint
								def point1 = o.vPoint1
						...
						case VALIDPOINT:
							...
							// Add card to selected pairs.
							chosenPairs[currentPair] = vPoint
							// Increment pairs.
							currentPair = currentPair + 1
							// Get card from square coordinates.
							...
							if ( matchOutcome == 2)
							{
								...
								switch (innerAlt.select()){
									case NEXT:
										...
										toController.write(new TurnManager (currentPlayer: myPlayerId))
										nextButton.read()
										nextPairConfig.write(" ")
										
										def p1 = chosenPairs[0]
										def p2 = chosenPairs[1]
										// Reverts picked cards to gray.
